...
`domains`: a list of domain information.
 ...
 - `r53_zone`: the route53 hosted zone name which contains `name`.
 - `countryName`: is used for countryName in the CSR.
 - `elb`: name of your Elastic Load Balancer name.
 - `elb_port`: ELB listening port. Default is 443.
 - `elb_region`: the region is which your elb has been deployed in. Default is the Lambda local  region.
 - `kmsKeyArn`: KMS key arn to encrypt letsencrypt account key and certificate private keys. You may also use `AES256` for AWS managed at rest encryption. Default is `AES256`.
